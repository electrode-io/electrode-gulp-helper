...
//
// from https://www.npmjs.com/package/run-sequence v1.2.2
//
// copied locally to turn off showStack for plugin errors to avoid nested tasks failures
// getting the same stack printed many times.
//
"use strict";
...
function runSequence(gulp) {
  ...
  var taskSets = Array.prototype.slice.call(arguments, 1).map(function (task) {
    ...
    finish = function (e) {
      ...
      var error;
      if (e && e.err) {
        error = new gutil.PluginError('run-sequence(' + e.task + ')', e.err, {showStack: false, showProperties: false});
      }
